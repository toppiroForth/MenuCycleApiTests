...
namespace MenuCycleApiTests_ci.MenuCycleItems.MenuCycleItemsStepDef
{
    ...
    public class MenuCycleItemCollectionsSteps
    {
        ...
        [When(@"user update all the items with day (.*) order (.*) menuid (.*) mealperiod (.*) and recipeid (.*)")]
        public void WhenUserUpdateAllTheItemsWithDayOrderMenuidMealperiodAndRecipeid(int _day, int _order, int _menuid, int _mealperiod, int recipeId)
        {
        ...
        public void ThenTheFollowingMenuCycleitemPropertieCollectionIsReturned(Table table)
        {
            ...
            // var listOfMenuCycles = JsonConvert.DeserializeObject<MenuCycleApiTests_ci.Domain.Siren.PagedCollectionEntity<MenuCycleItemsEntity<MenuCycleItemsEntityProperties>>>(response.Content);
            var listOfMenuCycles = JsonConvert.DeserializeObject<MenuCycleApiTests_ci.Domain.Siren.PagedCollectionEntity<MenuCycleItemEntity<MenuCycleItemsEntityProperties>>>(response.Content);
            table.CompareToSet<MenuCycleItemsEntityProperties>(listOfMenuCycles.entities.Select(x => x.properties));
        }

        [Then(@"user should get all menucyleitem id from the response")]
        public void ThenUserShouldGetAllMenucyleitemIdFromTheResponse()
        {
            var response = ScenarioContext.Current.Get<RestResponse>("Response");
            var listOfMenuCycles = JsonConvert.DeserializeObject<MenuCycleApiTests_ci.Domain.Siren.PagedCollectionEntity<MenuCycleItemEntity<MenuCycleItemsEntityProperties>>>(response.Content);
            // table.CompareToSet<MenuCycleItemsEntityProperties>(listOfMenuCycles.entities.Select(x => x.properties));
            foreach (var entitie in listOfMenuCycles.entities)
            {
                Console.WriteLine("This is the menucyleitem id " + entitie.properties.menuCycleItemId);
                menuCycleItemId.idList.Add(entitie.properties.menuCycleItemId);
            }
